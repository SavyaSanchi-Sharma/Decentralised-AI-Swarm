        ...
        loop {
                        ...
                        match msg {
                            Message::Weights(flat) => {
                                if let Ok(peer_model) = Model::from_flat_vec(&flat) {
                                    ...
                                    // prepare updated weights outside lock
                                    let updated = {
                            ...
                            Message::RequestSync => {
                                // serialize under short lock, drop before writing
                                let data = {
                                    let g = m_reader.lock().await;
                                    serde_json::to_string(
                                        &Message::Weights(g.to_flat_vec()),
                                    )
                                    .unwrap()
                                };
            ...
            tokio::spawn(async move {
                while let Ok(flat) = rx.recv().await {
                    let msg =
                        serde_json::to_string(&Message::Weights(flat)).unwrap();
                    let mut w = writer_clone.lock().await;
                    if let Err(e) = w
                        .write_all(format!("{}\n", msg).as_bytes())
                        .await
