...
use rand::prelude::*; // brings rng() and random() into scope
...
pub async fn run_client(id: &str, server_addr: &str) -> Result<()> {
    ...
    loop {
        ...
        // 2️⃣ Send weights to server
        let msg = Message::Weights(model.to_flat_vec()); // ✅ define `msg` here
        let json = serde_json::to_string(&msg)?;
        writer.write_all(format!("{}\n", json).as_bytes()).await?;
        writer.flush().await?;

        // 3️⃣ Receive updated global model
        line.clear();
        if reader.read_line(&mut line).await? > 0 {
            if let Ok(Message::Weights(flat)) =
                serde_json::from_str::<Message>(line.trim())
            {
